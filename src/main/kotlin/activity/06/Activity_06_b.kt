...
//              call function to check if the user still has 5 or more items from the library - Cannot borrow due to too many items still borrowed
    ...
    fun cheUserAccount(person: Person, borrowers: ArrayList<Borrower>) {
        ...
        checkUserStatus(noOfitemsPending, noOfUnpaiddues)
    }

    fun checkUserStatus(noOfitemsPending: Int, noOfUnpaiddues: Int) {
...
//            throw BookException("User cannot borrow due to too many items still borrowed!")
            throw LibraryExemption.UserStatusExemption.UserExceedBorrowLimitExemption("User cannot borrow due to too many items still borrowed!")
        }
//    if the user has unpaid dues - Cannot borrow Exception due to unpaid dues
        if (noOfUnpaiddues > 0) {
...
//            throw BookException("User Cannot borrow Exception due to unpaid dues!")
            throw LibraryExemption.UserStatusExemption.UserUnpaidDuesExemption("User Cannot borrow Exception due to unpaid dues!")
        }
    }
...
sealed class LibraryExemption(message: String): Exception(message){
    /**
        if the user still has 5 or more items from the library - Cannot borrow due to too many items still borrowed
        if the user has unpaid dues - Cannot borrow Exception due to unpaid dues
    */
    sealed class UserStatusExemption(message: String): Exception(message){
        class UserExceedBorrowLimitExemption(message: String): UserStatusExemption(message)
        class UserUnpaidDuesExemption(message: String): UserStatusExemption(message)
    }
    ...
    Function
        ...
        and etc
     */
    sealed class BookStatusExemption(message: String): Exception(message){
        /**
        ...
        */
        class BookInternalUseExemption(message: String): BookStatusExemption(message)
        class BookUnavailabilityExemption(message: String): BookStatusExemption(message)
        class BookUnderFixingStatusExemption(message: String): BookStatusExemption(message)
        class BookUnderReservedStatusExemption(message: String): BookStatusExemption(message)
    }
